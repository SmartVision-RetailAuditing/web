...
import { ArrowLeft, Store, User, Calendar, Activity, AlertTriangle, Package, BarChart2, Layers, Download } from 'lucide-react';
...
const TASK_TYPE_LABELS: Record<string, string> = {
  ...
  PLANOGRAM_COMPLIANCE: 'Planogram Compliance', PANORAMA: 'Panorama',
};
...
// ── Shelf Image Viewer (Before / After) ──────────────────────────────────────
const ShelfImageViewer = ({ preUrl, postUrl }: { preUrl: string; postUrl: string }) => {
  const [showPost, setShowPost] = useState(false);
  const activeUrl = showPost ? postUrl : preUrl;
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
      ...
      <div className="p-4 bg-gray-50 dark:bg-gray-800 flex justify-center">
        <img
          key={activeUrl}
          src={activeUrl}
          alt={showPost ? 'Post-audit shelf' : 'Pre-audit shelf'}
          className="w-full h-[600px] rounded-lg object-contain shadow transition-opacity duration-200"
...
const AuditDetailPage = () => {
  ...
  useEffect(() => {
    ...
    auditService.getAuditById(id)
      .then(setAudit)
      .catch((err: any) => setError(err.message || 'Audit could not be loaded.'))
  ...
  const handleExportPdf = async () => {
    ...
    setIsExporting(true);
    try {
      await auditService.exportPdf(audit.id);
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              Audit #{audit.id}<span className="mx-1.5">•</span>{TASK_TYPE_LABELS[audit.taskType] ?? audit.taskType}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Download size={15} />
            {isExporting ? 'Exporting...' : 'Export PDF'}
          </button>
          <span className={`px-3 py-1.5 rounded-full text-sm font-semibold border ${STATUS_STYLES[audit.status] ?? ''}`}>
      ...
      {/* Shelf Image */}
      {(audit.preImageUrl || audit.postImageUrl) && (
        <ShelfImageViewer
          preUrl={audit.preImageUrl}
          postUrl={audit.postImageUrl}
        />
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-bold text-gray-900 dark:text-white">Shelf Share</h2>
          </div>
          ...
          <div className="flex justify-between text-xs text-gray-400 mt-1">
            <span>0%</span><span>Target: 30%</span><span>100%</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-bold text-gray-900 dark:text-white">Brand Distribution</h2>
          </div>
          {brandData.length === 0 ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800">
            ...
            <p className="text-xs text-gray-400">
              {audit.products.length} product{audit.products.length > 1 ? 's' : ''} identified
            </p>
          </div>
                  ...
                  <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-3 text-right">
                      <span className={`text-xs font-bold ${
                        product.confidenceScore >= 0.9
                          ? 'text-green-600 dark:text-green-400'
                          : product.confidenceScore >= 0.75
                    ...
                    <td className="px-6 py-3 text-center">
                      {product.isManuallyEdited ? (
                        <span className="text-xs text-orange-600 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-900/30 px-2 py-0.5 rounded-full font-medium">
                          Manual
                        </span>
                      ) : (
