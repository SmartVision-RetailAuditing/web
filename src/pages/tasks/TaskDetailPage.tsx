...
const TaskDetailPage = () => {
  ...
  if (error || !task) return <div className="p-12 text-center text-red-500 text-sm">{error || 'Task not found.'}</div>;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm space-y-1">
            ...
            <InfoRow icon={<ClipboardList size={15} />} label="Task Type" value={
              <span className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-2 py-1 rounded-md font-medium">
                {TASK_TYPE_LABELS[task.taskType] ?? task.taskType}
              </span>
            } />
            <InfoRow icon={<Calendar size={15} />} label="Due Date" value={
              <span className={isOverdue ? 'text-red-500 font-medium' : ''}>
                {new Date(task.dueDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}
              </span>
            } />
            {task.completedAt && (
              <InfoRow icon={<CheckCircle2 size={15} />} label="Completed At" value={
                new Date(task.completedAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })
              } />
            )}
            {task.description && (
              <InfoRow icon={<Clock size={15} />} label="Description" value={
                <span className="text-gray-600 dark:text-gray-400">{task.description}</span>
              } />
            )}
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
            ...
            {task.assigneeId && task.assigneeName ? (
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center text-base font-bold shrink-0">
                  {task.assigneeName.charAt(0).toUpperCase()}
                </div>
                <div>
                  <div className="font-medium text-gray-900 dark:text-white">{task.assigneeName}</div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-medium text-gray-900 dark:text-white">{task.storeName}</p>
