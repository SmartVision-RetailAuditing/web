...
const AssignTaskModal: React.FC<AssignTaskModalProps> = ({
  ...
  const fetchUnassignedTasks = useCallback(async (search: string) => {
    ...
    try {
      ...
      setError('Task listesi yüklenemedi.');
    } finally {
  ...
  const fetchWorkers = useCallback(async (search: string) => {
    ...
    try {
      ...
      setError('Field worker listesi yüklenemedi.');
    } finally {
  ...
  useEffect(() => {
    if (isOpen && step === 'task') fetchUnassignedTasks(debouncedTaskSearch);
  }, [debouncedTaskSearch, step]);

  useEffect(() => {
    if (isOpen && step === 'worker') fetchWorkers(debouncedWorkerSearch);
  }, [debouncedWorkerSearch, step]);
  ...
  const activeTaskType  = selectedTask
          ...
          {step === 'task' && (
            <>
              <div className="relative">
                ...
                <input
                  type="text" placeholder="Search unassigned tasks..."
              ...
              <div className="max-h-72 overflow-y-auto space-y-1.5">
                {tasksLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-300 text-xs mt-1">All tasks have been assigned.</p>
                  </div>
                ) : tasks.map(task => (
                  <button
                    key={task.id}
                    onClick={() => handleTaskSelect(task)}
                    className="w-full flex items-center justify-between p-3 rounded-lg border border-gray-100 hover:border-blue-200 hover:bg-blue-50/50 transition-colors text-left group"
                  >
                    ...
                    <ChevronRight size={16} className="text-gray-300 group-hover:text-blue-400 shrink-0 ml-2 transition-colors" />
                  </button>
                ))}
              </div>
            </>
              ...
              {activeTaskLabel && (
                <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg border border-blue-100">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-blue-800 truncate">{activeTaskLabel}</p>
                ...
                {workersLoading ? (
                  ...
                  >
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${
                      selectedWorker?.id === worker.id ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'
                    }`}>
